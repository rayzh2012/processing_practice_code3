...
/*   =================================================================================       
         ...
         // fill(GraphColor);
          ...
          if(from<0){                                      // If the From or To value is out of bounds 
           for (int x=0; x<a.length; x++){                 // of the array, adjust them 
               rect(int(xPos+x*float(Width)/(a.length)),//x-coordinate
                    yPos+Height-2,//y coordinate
                    Width/a.length-2,// width
                    -a[x]/(yMax-yMin)*Height);//height
                    /////////
                    switch(x) {
                    case 1: fill(204, 102, 0);
                    break;
  ...
  void Bar(float[] a ) {
   //  Chart myChart;
     //     myChart = cp5.addChart("amuse")
       //        .setData("amuse", a[1])
         //      .setPosition(xPos,yPos)
           //    .setSize(int(Width/a.length-2), int(-a[1]/(yMax-yMin)*Height))
             //  .setRange(-20, 20)
               //.setColors("amuse", color(255,0,255),color(255,0,0))
               //.setView(Chart.BAR) // use Chart.LINE, Chart.PIE, Chart.AREA, Chart.BAR_CENTERED
              // ;
         // push: add data from right to left (last in)
 //    myChart.push("amuse", (sin(frameCount*0.1)*10));
          stroke(GraphColor);
          fill(GraphColor);
  ...
  for (int x=0; x<a.length; x++){                 // of the array, adjust them 
               rect(int(xPos+x*float(Width)/(a.length)),
                    ...
                    switch(x) {
                    case 0:
                     text("Amuse",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                     fill(0,0,5);
                   break;
                   case 1:
                     textSize(18);
                     fill(0);
                    text("Anger",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                     fill(223,100,100);
                   break;
                   case 2:
                    text("Awe",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                   fill(100,100,100);
                   break;
                   case 3: 
                    text("Content",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                   fill(0,255,255);
                   break;
                   case 4:
                   text("Disgust",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                   fill(255,0,255);
                   break;
                   case 5:
                   text("Excite",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                   fill(255,0,0);
                   break;
                   case 6:
                   text("Fear",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                   fill(0,255,0);
                   break;
                   case 7:
                   text("Sad",xPos+x*float(Width)/(a.length)+80,yPos+Height+20);
                  fill(0,5,0);
                   break;
                   case 8:
                   fill(0,0,0);

...
public class Graph {
  ...
  public void DrawCenter(int yearIndex){
    ...
    textFont(myFontB);
    textSize(48);
    text("30% Drowsy",width/2,height/2+14);
    rectMode(CORNER);
    textFont(myFont);
    ...
    for(int i=0; i<numReasons; i++){
     ...
     bWidth = (float)(myDataSet[yearIndex][i][orgIndex])/totalYearOrgRemovals*(aEnd-aStart)*scalingFactor; // in radians
      ...
      for(int j=1; j<myGraph.ringWidth; j++){
       ...
       arc(myGraph.x,myGraph.y,myGraph.outerDiameter-(i*myGraph.ringWidth)-j-1,myGraph.outerDiameter-(i*myGraph.ringWidth)-j-1,bStart,bStart+(aEnd-bStart)/k);
        }  
    }
    }
  }
